package com.jack.player.builder;
...
public class VideoOptionBuilder {
    ...
    //是否自动旋转和mNeedLockFull有冲突最好自动选择mLockCurScreen
    ...
    //是否播放器当失去音频焦点，true：释放播放器 ，false：暂停播放
    ...
    //是否只支持横屏设置
    protected boolean mIsOnlyRotateLand = false;
     ...
     *
     * @param cachePath 默认：内部存储：data/data/pkg/cache/video-cache/中
     */
    ...
    public void build(StandardVideoPlayer standardVideoPlayer) {
        if (mProgressBarDrawable != null && mProgressBarThumbDrawable != null) {
            standardVideoPlayer.setBottomShowProgressBarDrawable(mProgressBarDrawable, mProgressBarThumbDrawable);
        }
